...
* **Refactor**: What happen when we want to refactor our code? We are in big trouble. Even when we decide to not break the API, maybe that internal changes are not working properly on some edge cases where it worked in the past. That will break the application for some people and they won't be happy at all (and that kind of bugs are normally a pain in the ass).
...
Did you start seeing what are we getting here so far? **API design**. By using our object before we coded it, we are using the `API` as we would like to use it. That is a much much better way to define our `API`.
...
Yes it does! This in an example of not **extra code**. We coded the minimum necessary to make it work, and well, that is what we need at this point.
...
{% img /images/posts/introtest/image6.png %}
...
We are smart enough to make a proper first version which actually pass.

Now, for non exact divisions, we want to round the result, we doesn't want any decimals.
...
This test is a little bit different. Instead of passing a variable to `expect` we are passing a function. We expect that call to end on a exception so saving the result as we previously did won't work (We expect to never return that result but throw an exception). We also use the `toThrow` function on Jasmine.
...
Uh, we did a **refactor** and we didn't broke anything.

I will leave the other two calculator functions as an exercise.

### Conclusions of this example

Even when it is really really simple example. We already saw how can we address those problems I described earlier:
...
Maybe you won't notice it with this example, but with proper tests, you will save a lot of hours maintaining **extra code**, dealing with **API design** with hopefully won't end on breaking changes, **refactor**ing code without fear and of course and being sure that your code **will work**.
...
The rest is not new. Once we have our modules loaded and our services injected and saved in local variables, we just need to do some testings. For that, we need to wait for the next articles of this serie.